...
agent-lab is a local web app (Korean UI, README in Korean) for handing an LLM tools and watching it work.
It has three panels:

- **도구 (Tools)** — three built-in memo tools (`memo_tool_1/2/3`: search/save/delete). Their
  descriptions and role prompts can be edited live from the screen.
...
`data/notes.json` on purpose — mixing them would make the experiment agent's tool log noisy with
irrelevant memo-file reads. Only the experiment role gets this tool; the orchestrator must ask it to read
values rather than inventing business assumptions itself.
...
| `lab/tools.py` | built-in memo tools |
